...
module.exports = cds.service.impl(async function () {
  if (this.name !== 'fisc.FiscalizationService') {
    console.log(`ℹ️ handler loaded for ${this.name}, skipping (target fisc.FiscalizationService)`);
    return;
  }
  console.log('✅ fisc.FiscalizationService handlers registered');

  this.on('submitFromEvent', async (req) => {
  ...
  this.on('ackEOR', async (req) => {
    ...
    const { Invoices, Responses } = this.entities;
